...
import { RejectQuoteUseCase } from '../RejectQuoteUseCase.js';
...
describe('RejectQuoteUseCase', () => {
  it('rejects a PENDING quote and saves it', async () => {
    const quote = makeQuote();
    vi.mocked(mockGateway.findById).mockResolvedValue(quote);
    vi.mocked(mockGateway.save).mockImplementation(async (q) => q);

    const result = await new RejectQuoteUseCase(mockGateway).execute({ quoteId: toUUID('q-1') });
  ...
  it('throws QuoteNotFoundException when quote does not exist', async () => {
    ...
    await expect(
      new RejectQuoteUseCase(mockGateway).execute({ quoteId: toUUID('q-missing') }),
  ...
  it('propagates QuoteAlreadyProcessedException from domain', async () => {
    const quote = makeQuote(QuoteStatus.REJECTED);
    vi.mocked(mockGateway.findById).mockResolvedValue(quote);

    await expect(
      new RejectQuoteUseCase(mockGateway).execute({ quoteId: toUUID('q-1') }),
